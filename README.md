...
heroku config:set GOOGLE_APPLICATION_CREDENTIALS_JSON='<service-account-json>'
```
...
専用toolは公式CSVキャッシュを優先し、キャッシュがない場合のみ `data/*.json` の少数サンプルにフォールバックします。
...
起動時取得は公式サイトへのアクセスとCSVパースが発生するため、起動時間が長くなります。安定運用では外部ストレージへの移行を検討してください。
